...
CREATE OR REPLACE FUNCTION tracked_changes_tpdm.actionstepdescriptor_deleted()
    RETURNS trigger AS
$BODY$
BEGIN
    INSERT INTO tracked_changes_edfi.descriptor(olddescriptorid, oldcodevalue, oldnamespace, id, discriminator, changeversion)
    SELECT OLD.ActionStepDescriptorId, b.codevalue, b.namespace, b.id, 'tpdm.ActionStepDescriptor', nextval('changes.ChangeVersionSequence')
    FROM edfi.descriptor b WHERE old.ActionStepDescriptorId = b.descriptorid ;

    RETURN NULL;
END;
$BODY$ LANGUAGE plpgsql;

IF NOT EXISTS(SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'trackdeletes' AND event_object_schema = 'tpdm' AND event_object_table = 'actionstepdescriptor') THEN
CREATE TRIGGER TrackDeletes AFTER DELETE ON tpdm.actionstepdescriptor 
    FOR EACH ROW EXECUTE PROCEDURE tracked_changes_tpdm.actionstepdescriptor_deleted();
END IF;
