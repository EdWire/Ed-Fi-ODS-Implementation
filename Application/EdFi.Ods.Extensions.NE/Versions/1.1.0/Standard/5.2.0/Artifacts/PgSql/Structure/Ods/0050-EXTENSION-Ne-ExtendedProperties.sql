...
COMMENT ON COLUMN ne.CalendarDateExtension.Date IS 'The month, day, and year of the calendar event.';
COMMENT ON COLUMN ne.CalendarDateExtension.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
...
COMMENT ON COLUMN ne.DisciplineActionExtension.DisciplineActionIdentifier IS 'Identifier assigned by the education organization to the discipline action.';
COMMENT ON COLUMN ne.DisciplineActionExtension.DisciplineDate IS 'The date of the discipline action.';
...
COMMENT ON COLUMN ne.DisciplineIncidentExtension.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
...
COMMENT ON TABLE ne.PartCTransitionDelayReasonDescriptor IS 'The reason the Part C IDEA child was delayed in transitioning to Part B.';
COMMENT ON COLUMN ne.PartCTransitionDelayReasonDescriptor.PartCTransitionDelayReasonDescriptorId IS 'A unique identifier used as Primary Key, not derived from business logic, when acting as Foreign Key, references the parent table.';
...
COMMENT ON COLUMN ne.PostGraduateActivity.LocalEducationAgencyId IS 'The identifier assigned to a local education agency. It must be distinct from any other identifier assigned to educational organizations, such as a SchoolId, to prevent duplication.';
...
COMMENT ON COLUMN ne.SectionDelivery.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
...
COMMENT ON COLUMN ne.SectionDelivery.SessionName IS 'The identifier for the calendar for the academic session.';
...
COMMENT ON COLUMN ne.SectionExtension.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
...
COMMENT ON COLUMN ne.SectionExtension.SessionName IS 'The identifier for the calendar for the academic session.';
...
COMMENT ON TABLE ne.StudentDisciplineIncidentBehaviorAssociationExtension IS '';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.BehaviorDescriptorId IS 'Describes behavior by category.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.IncidentIdentifier IS 'A locally assigned unique identifier (within the school or school district) to identify each specific DisciplineIncident or occurrence. The same identifier should be used to document the entire discipline incident even if it included multiple offenses and multiple offenders.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.StudentUSI IS 'A unique alphanumeric code assigned to a student.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.GunFreeSchoolViolation IS 'True = Student was charged with a Gun-Free Schools Violation.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.ReferralToLawEnforcement IS 'Indicates if a student was referred to law enforcement.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.SchoolRelatedArrestIndicator IS 'Indicates if student was involved in a disciplinary incident that resulted in an arrest.';
COMMENT ON COLUMN ne.StudentDisciplineIncidentBehaviorAssociationExtension.SeriousBodilyInjury IS 'Indicates if an incident resulted in serious bodily injury as defined by 18 U.S.C. Section 1365(3)(h).';
...
COMMENT ON COLUMN ne.StudentEarlyLearningProgramAssociation.BeginDate IS 'The earliest date the student is involved with the program. Typically, this is the date the student becomes eligible for the program.  Note: Date interpretation may vary. Ed-Fi recommends inclusive dates, but states may define dates as inclusive or exclusive. For calculations, align with local guidelines.';
...
COMMENT ON COLUMN ne.StudentLanguageInstructionProgramAssociationExtension.BeginDate IS 'The earliest date the student is involved with the program. Typically, this is the date the student becomes eligible for the program.  Note: Date interpretation may vary. Ed-Fi recommends inclusive dates, but states may define dates as inclusive or exclusive. For calculations, align with local guidelines.';
...
COMMENT ON COLUMN ne.StudentLanguageInstructionProgramAssociationExtension.DismissedViaIEPReview IS 'A special education student that is no longer considered an English Learner due to a decision by the IEP review team to dismiss the ELPA testing requirement.';
...
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.EntryDate IS 'The month, day, and year on which an individual enters and begins to receive instructional services in a school for each school year. The EntryDate value should be the date the student enrolled, or when the student''s enrollment materially changed, such as with a grade promotion.  Note: Date interpretation may vary. Ed-Fi recommends inclusive dates, but states may define dates as inclusive or exclusive. For calculations, align with local guidelines.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.SchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.StudentUSI IS 'A unique alphanumeric code assigned to a student.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.ExpectedHighSchoolOfGraduationSchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.ReportingSchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.ResidentLocalEducationAgencyId IS 'The identifier assigned to a local education agency. It must be distinct from any other identifier assigned to educational organizations, such as a SchoolId, to prevent duplication.';
COMMENT ON COLUMN ne.StudentSchoolAssociationExtension.ResidentSchoolId IS 'The identifier assigned to a school. It must be distinct from any other identifier assigned to educational organizations, such as a LocalEducationAgencyId, to prevent duplication.';
...
COMMENT ON COLUMN ne.StudentSpecialEducationProgramAssociationExtension.PartCTransitionDelayReasonDescriptorId IS 'The reason the Part C IDEA child was delayed in transitioning to Part B.';
